...
public class FusionZoneContents implements IInventory {
     ...
     */
    public static FusionZoneContents createForTileEntity(int size,
                                                          Predicate<PlayerEntity> canPlayerAccessInventoryLambda,
                                                          Notify markDirtyNotificationLambda) {
        return new FusionZoneContents(size, canPlayerAccessInventoryLambda, markDirtyNotificationLambda);
    }
     ...
     * @return the new ChestContents
     */
    public static FusionZoneContents createForClientSideContainer(int size) {
        return new FusionZoneContents(size);
    }
    ...
    public CompoundNBT serializeNBT()  {
        return fusionComponentContents.serializeNBT();
    }
    ...
    public void deserializeNBT(CompoundNBT nbt)   {
        fusionComponentContents.deserializeNBT(nbt);
    }
    ...
    public boolean isItemValidForSlot(int index, ItemStack stack) {
        return fusionComponentContents.isItemValid(index, stack);
    }
    ...
    public int getSizeInventory() {
        return fusionComponentContents.getSlots();
    }

    @Override
    public boolean isEmpty() {
        for (int i = 0; i < fusionComponentContents.getSlots(); ++i) {
            if (!fusionComponentContents.getStackInSlot(i).isEmpty()) return false;
        }
        return true;
    }
    ...
    public ItemStack getStackInSlot(int index) {
        return fusionComponentContents.getStackInSlot(index);
    }

    @Override
    public ItemStack decrStackSize(int index, int count) {
        if (count < 0) throw new IllegalArgumentException("count should be >= 0:" + count);
        return fusionComponentContents.extractItem(index, count, false);
    }

    @Override
    public ItemStack removeStackFromSlot(int index) {
        int maxPossibleItemStackSize = fusionComponentContents.getSlotLimit(index);
        return fusionComponentContents.extractItem(index, maxPossibleItemStackSize, false);
    }

    @Override
    public void setInventorySlotContents(int index, ItemStack stack) {
        fusionComponentContents.setStackInSlot(index, stack);
    }

    @Override
    public void clear() {
        for (int i = 0; i < fusionComponentContents.getSlots(); ++i) {
            fusionComponentContents.setStackInSlot(i, ItemStack.EMPTY);
        }
    }
    ...
    public ItemStack increaseStackSize(int index, ItemStack itemStackToInsert) {
        ItemStack leftoverItemStack = fusionComponentContents.insertItem(index, itemStackToInsert, false);
    ...
    public boolean doesItemStackFit(int index, ItemStack itemStackToInsert) {
        ItemStack leftoverItemStack = fusionComponentContents.insertItem(index, itemStackToInsert, true);
        return leftoverItemStack.isEmpty();
    }

    // ---------

    private FusionZoneContents(int size) {
        this.fusionComponentContents = new ItemStackHandler(size);
    }

    private FusionZoneContents(int size, Predicate<PlayerEntity> canPlayerAccessInventoryLambda, Notify markDirtyNotificationLambda) {
        this.fusionComponentContents = new ItemStackHandler(size);
        this.canPlayerAccessInventoryLambda = canPlayerAccessInventoryLambda;
        this.markDirtyNotificationLambda = markDirtyNotificationLambda;
    }
    ...
    private final ItemStackHandler fusionComponentContents;
}
